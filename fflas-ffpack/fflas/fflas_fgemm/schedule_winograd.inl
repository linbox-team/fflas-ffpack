...
namespace FFLAS { namespace BLAS3 {
                         ...
                         )
    {
        ...
        WH.Outmax = std::max (U1Max, std::max (U5Max, std::max (U6Max, U7Max)));
                ...
                // Schedule using 2 temps and 17 ops (merging 4 adds in muladds)
    template < class Field, class FieldTrait >
    inline void Winograd_muladd (const Field& F,
                                 const FFLAS_TRANSPOSE ta,
                                 const FFLAS_TRANSPOSE tb,
                                 const size_t mr, const size_t nr, const size_t kr,
                                 const typename Field::Element alpha,
                         ...
                         )
    {
        ...
        // P1 = alpha . A11 * B11 in C11
        MMH_t H1(F, WH.recLevel-1, WH.Amin, WH.Amax, WH.Bmin, WH.Bmax, 0, 0);
        fgemm (F, ta, tb, mr, nr, kr, alpha, A11, lda, B11, ldb, F.zero, C11, ldc, H1);

        // C12 <- P1
        fassign (F, mr, nr, C11, ldc, C12, ldc);
        ...
        // U2 = P1 + P6 = P1 + alpha . S2 * T2 in C12
        MMH_t H6(F, WH.recLevel-1, 2*WH.Amin-WH.Amax, 2*WH.Amax-WH.Amin, 2*WH.Bmin-WH.Bmax, 2*WH.Bmax-WH.Bmin, H1.Outmin, H1.Outmax);
        fgemm (F, ta, tb, mr, nr, kr, alpha, X1, ldX1, X2, ldX2, F.one, C12, ldc, H6);

        // U3 = P7 + U2 in C21
        DFElt U3Min, U3Max;
        // This test will be optimized out
        if (Protected::NeedPreAddReduction(U3Min, U3Max, H7.Outmin, H7.Outmax, H6.Outmin, H6.Outmax, WH)){
            freduce (F, mr, nr, C12, ldc);
            freduce (F, mr, nr, C21, ldc);
        }
        faddin (DF,mr,nr,(DFCEptr)C12,ldc, (DFEptr)C21,ldc);
        ...
        DFElt U4Min, U4Max;
        // This test will be optimized out
        if (Protected::NeedPreAddReduction(U4Min, U4Max, H6.Outmin, H6.Outmax, H5.Outmin, H5.Outmax, WH)){
            freduce (F, mr, nr, C22, ldc);
            freduce (F, mr, nr, C12, ldc);
        }
        faddin (DF,mr,nr,(DFCEptr)C22,ldc, (DFEptr)C12,ldc);
        ...
        // U5 = U4 + P3 = U4 + alpha . S4*B22 in C12
        MMH_t H3(F, WH.recLevel-1, 2*WH.Amin-2*WH.Amax, 2*WH.Amax-2*WH.Amin, WH.Bmin, WH.Bmax, U4Min, U4Max);
        fgemm (F, ta, tb, mr, nr, kr, alpha, X1, ldX1, B22, ldb, F.one, C12, ldc, H3);
        ...
        DFElt U7Min, U7Max;
        // This test will be optimized out
        if (Protected::NeedPreAddReduction (U7Min,U7Max, H5.Outmin,H5.Outmax, U3Min, U3Max, WH) ){
            freduce (F, mr, nr, C21, ldc);
            freduce (F, mr, nr, C22, ldc);
        }
        faddin(DF,mr,nr,(DFCEptr)C21,ldc,(DFEptr)C22,ldc);
        ...
        // U6 = U3 - P4 =U3 -  alpha . A22 * T4 in C21
        typename Field::Element malpha; F.init(malpha); F.neg (malpha, alpha);
        MMH_t H4(F, WH.recLevel-1, WH.Amin, WH.Amax, 2*WH.Bmin-2*WH.Bmax, 2*WH.Bmax-2*WH.Bmin, U3Min, U3Max);
        fgemm (F, ta, tb, mr, nr, kr, malpha, A22, lda, X2, ldX2, F.one, C21, ldc, H4);

        fflas_delete (X2);

        // U1 = P1 + P2 = P1 + alpha . A12 * B21  in C11
        MMH_t H2(F, WH.recLevel-1, WH.Amin, WH.Amax, WH.Bmin, WH.Bmax, H1.Outmin, H1.Outmax);
        fgemm (F, ta, tb, mr, nr, kr, alpha, A12, lda, B21, ldb, F.one, C11, ldc, H2);

        WH.Outmin = std::min (H2.Outmin, std::min (H3.Outmin, std::min (H4.Outmin, U7Min)));
        WH.Outmax = std::max (H2.Outmax, std::max (H3.Outmax, std::max (H4.Outmax, U7Max)));
