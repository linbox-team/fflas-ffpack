...
#define __FFLASFFPACK_fflas_sparse_coo_utils_INL
 ...
 namespace FFLAS{
	...
	inline void sparse_delete(const Sparse<Field, SparseMatrix_t::COO> & A){
		fflas_delete(A.dat);
		fflas_delete(A.col);
		fflas_delete(A.row);
	}
	...
	template<class Field>
	inline void sparse_delete(const Sparse<Field, SparseMatrix_t::COO_ZO> & A){
		fflas_delete(A.col);
		fflas_delete(A.row);
	}

	template<class Field, class IndexT>
	inline void sparse_init(const Field & F, Sparse<Field, SparseMatrix_t::COO> & A, const IndexT * row,
						const IndexT * col, typename Field::ConstElement_ptr dat, uint64_t rowdim,
	...
	template<class Field, class IndexT>
	inline void sparse_init(const Field & F, Sparse<Field, SparseMatrix_t::COO_ZO> & A, const IndexT * row,
						const IndexT * col, typename Field::ConstElement_ptr dat, uint64_t rowdim,
						uint64_t coldim, uint64_t nnz){
		A.kmax = Protected::DotProdBoundClassic(F,F.one);
		A.m = rowdim;
