...
namespace FFLAS{
	...
	inline void pfspmv(const Field & F, const Sparse<Field, SparseMatrix_t::HYB_ZO> & A, typename Field::ConstElement_ptr x,
		...
		if(A.one != nullptr)
			sparse_details_impl::pfspmv(F, *(A.one), x, y, [F](Element & a, const Element & b){F.addin(a, b);}, FieldCategories::GenericTag());
		if(A.mone != nullptr)
			sparse_details_impl::pfspmv(F, *(A.mone), x, y, [F](Element & a, const Element & b){F.subin(a, b);}, FieldCategories::GenericTag());
		if(A.dat != nullptr)
	...
	inline void pfspmv(const Field & F, const Sparse<Field, SparseMatrix_t::HYB_ZO> & A, typename Field::ConstElement_ptr x,
		...
		using Element = typename Field::Element;
		if(A.one != nullptr)
			sparse_details_impl::pfspmv(F, *(A.one), x, y, [F](Element & a, const Element & b){F.addin(a, b);}, FieldCategories::UnparametricTag());
		if(A.mone != nullptr)
			sparse_details_impl::pfspmv(F, *(A.mone), x, y, [F](Element & a, const Element & b){F.subin(a, b);}, FieldCategories::UnparametricTag());
		if(A.dat != nullptr)
