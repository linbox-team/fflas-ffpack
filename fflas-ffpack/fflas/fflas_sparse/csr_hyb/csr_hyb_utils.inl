...
namespace FFLAS{
	...
	inline void sparse_init(const Field & F, Sparse<Field, SparseMatrix_t::CSR_HYB> & A,
		...
		std::sort(data.begin(), data.end(), [&F](const coo & a, const coo & b){
			return (a.row < b.row) ||
				   ...
				   ((a.row == b.row) && (!(F.isOne(a.val) && F.isMOne(a.val)) == !(F.isOne(b.val) && F.isMOne(b.val))) && (a.col < b.col));
		});
		...
		for(uint64_t i = 0 ; i < rowdim ; ++i)
        	cout << "(" << A.st[4*i] << " , " << A.st[4*i+1] << " , " << A.st[4*i+2] << " , " << A.st[4*i+3] << ") " << endl;
		cout << endl;
		cout << endl;
		for(uint64_t i = 0 ; i < rowdim ; ++i){
