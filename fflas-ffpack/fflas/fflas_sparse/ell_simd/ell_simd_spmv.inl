...
template <class Field>
inline void
fspmv(const Field &F, const Sparse<Field, SparseMatrix_t::ELL_simd> &A,
...
inline void
fspmv(const Field &F, const Sparse<Field, SparseMatrix_t::ELL_simd_ZO> &A,
      typename Field::ConstElement_ptr x, typename Field::Element_ptr y,
        ...
        for (; i < A.ld; ++j) {
            ...
            for (; k < ROUND_DOWN(A.chunk, 4); k += 4) {
                ...
                func(y[i * A.chunk + k + 1],
                     x[A.col[start + j * A.chunk + k + 1]]);
                func(y[i * A.chunk + k + 2],
                     x[A.col[start + j * A.chunk + k + 2]]);
                func(y[i * A.chunk + k + 3],
                     x[A.col[start + j * A.chunk + k + 3]]);
            }
            for (; k < A.chunk; ++k)
                func(y[i * A.chunk + k], x[A.col[start + j * A.chunk + k]]);
        }
