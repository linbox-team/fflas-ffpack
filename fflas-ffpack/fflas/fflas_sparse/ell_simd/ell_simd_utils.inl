...
namespace FFLAS{
	...
	inline void sparse_init(const Field & F, Sparse<Field, SparseMatrix_t::ELL_simd> & A,
		...
		// cout << A.m << " " << ROUND_DOWN(A.m, simd::vect_size)+simd::vect_size << " " << m/A.chunk << endl;
		A.nChunks = m/A.chunk;
	...
	template<class Field, class IndexT>
	inline void sparse_init(const Field & F, Sparse<Field, SparseMatrix_t::ELL_simd_ZO> & A,
		const IndexT * row, const IndexT * col, typename Field::ConstElement_ptr dat,
		 uint64_t rowdim, uint64_t coldim, uint64_t nnz){
		#ifdef __FFLASFFPACK_USE_SIMD
		using simd = Simd<typename Field::Element>;
