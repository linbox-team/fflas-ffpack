...
namespace sell_details{
	...
	inline void sparse_init(const Field & F, Sparse<Field, SparseMatrix_t::SELL> & A,
		 ...
		 uint64_t rowdim, uint64_t coldim, uint64_t nnz, uint64_t sigma = 0){
		...
		A.maxrow = (std::max_element(infos.begin(), infos.end(), [](const Info & a, const Info & b){
		...
		for(; it < ROUND_DOWN(rowdim, sigma) ; it+=sigma){
			std::sort(infos.begin()+it, infos.begin()+it+sigma, [](const Info & a, const Info & b){
				return a.size >= b.size;
			});
		}
		if(it != rowdim){
			std::sort(infos.begin()+it, infos.end(), [](Info a, Info b){
				return a.size >= b.size;
			});
		}

		// cout << "sorted : " << std::is_sorted(infos.begin(), infos.end(), [](Info a, Info b){
		// 	return a.size >= b.size;
		// }) << endl;

		for(size_t i = 0 ; i < infos.size() ; ++i){
			if(infos[i].begin > nnz)
				cout << "ERROR sort " << i << " size : " << infos[i].size << " begin : " << infos[i].begin << " perm : " << infos[i].perm << endl;
		}
  ...
  //       cout << endl;
	...
	template<class Field, class IndexT>
	inline void sparse_init(const Field & F, Sparse<Field, SparseMatrix_t::SELL_ZO> & A,
		const IndexT * row, const IndexT * col, typename Field::ConstElement_ptr dat,
