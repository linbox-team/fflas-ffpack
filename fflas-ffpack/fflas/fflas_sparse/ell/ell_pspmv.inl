...
inline void pfspmv(const Field &F, const Sparse<Field, SparseMatrix_t::ELL> &A,
    ...
    tbb::parallel_for(tbb::blocked_range<index_t>(0, A.m, step),
        ...
        for (index_t i = r.begin(), end = r.end(); i < end; ++i) {
            ...
            for (; j < ROUND_DOWN(A.ld, 4); j += 4) {
                ...
                F.axpyin(y2, A.dat[i * A.ld + j + 1],
                         x[A.col[i * A.ld + j + 1]]);
                F.axpyin(y3, A.dat[i * A.ld + j + 2],
                         x[A.col[i * A.ld + j + 2]]);
                F.axpyin(y4, A.dat[i * A.ld + j + 3],
                         x[A.col[i * A.ld + j + 3]]);
            }
            for (; j < A.ld; ++j) {
                F.axpyin(y1, A.dat[i * A.ld + j], x[A.col[i * A.ld + j]]);
...
inline void
pfspmv(const Field &F, const Sparse<Field, SparseMatrix_t::ELL_ZO> &A,
