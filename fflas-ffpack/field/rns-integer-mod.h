...
namespace FFPACK {
	...
	public: 
		...
		RNSIntegerMod(const integer& p, const RNS& rns) : _p(p),
								  _Mi_modp_rns(rns._size*rns._size), 
								  _iM_modp_rns(rns._size*rns._size),
								  _rns(&rns),
								  _F(p){			
			init(one,1);
			init(zero,0);
			init(mOne,-1);
			integer iM=0;
			size_t size=rns._size;
			integer sum=0;			
			for (size_t i=0;i<size;i++){
				integer Mi = rns._Mi[i] % _p;						
				for (size_t j=0;j<size;j++){
					_iM_modp_rns[i+j*size]=  iM % rns._basis[j];
					_Mi_modp_rns[i+j*size]=  Mi % rns._basis[j];
				}
				iM+=rns._M;iM%=_p;			
				sum+=rns._basis[i];
			}
