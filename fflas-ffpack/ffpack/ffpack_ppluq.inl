...
namespace FFPACK {
	...
	pPLUQ(const Field& Fi, const FFLAS::FFLAS_DIAG Diag,
	      ...
	      size_t* P, size_t* Q)
	  {
		  ...
		  // [ B1 ] <- P1^T A2                          
		  // [ B2 ]
		  TASK(READ(Fi, P1), NOWRITE(), READWRITE(A2), papplyP, Fi, FflasLeft, FflasNoTrans, N-N2, 0, M2, A2, lda, P1);
		  //applyP( Fi, FflasLeft, FflasNoTrans, N-N2, 0, M2, A2, lda, P1);
		  
		  // [ C1 C2 ] <- A3 Q1^T                                          
		  TASK(READ(Fi, Q1), NOWRITE(), READWRITE(A3), papplyP, Fi, FflasRight, FflasTrans, M-M2, 0, N2, A3, lda, Q1);
		  //papplyP( Fi, FflasRight, FflasTrans, M-M2, 0, N2, A3, lda, Q1);
		  
		  WAIT;
		  // D <- L1^-1 B1            
		  TASK(READ(Fi, A, R1), NOWRITE(), READWRITE(A2), pftrsm, Fi, FflasLeft, FflasLower, FflasNoTrans, OppDiag, R1, N-N2, Fi.one, A, lda, A2, lda,  method, NUM);
		  ...
		  // E <- C1 U1^-1  
		  TASK(READ(Fi, R1, A), NOWRITE(), READWRITE(A3), pftrsm,Fi, FflasRight, FflasUpper, FflasNoTrans, Diag, M-M2, R1, Fi.one, A, lda, A3, lda,  method, NUM);
		  ...
		  WAIT;
		  // F <- B2 - M1 D 
		  TASK(READ(Fi, A), NOWRITE(), READWRITE(A2), pfgemm, Fi, FflasNoTrans, FflasNoTrans, M2-R1, N-N2, R1, Fi.mOne, A + R1*lda, lda, A2, lda, Fi.one, A2+R1*lda, lda, method, HPAC_NUM_THREADS);
		  //fgemm( Fi, FflasNoTrans, FflasNoTrans, M2-R1, N-N2, R1, Fi.mOne, A + R1*lda, lda, A2, lda, Fi.one, A2+R1*lda, lda);
		  
		  // G <- C2 - E V1
		  TASK(READ(Fi, R1, A), NOWRITE(), READWRITE(A3), pfgemm, Fi, FflasNoTrans, FflasNoTrans, M-M2, N2-R1, R1, Fi.mOne, A3, lda, A+R1, lda, Fi.one, A3+R1, lda,  method, HPAC_NUM_THREADS);
		  ...
		  // H <- A4 - ED
		  TASK(READ(Fi, M2, N2, R1, A3, A2), NOWRITE(), READWRITE(A4), pfgemm, Fi, FflasNoTrans, FflasNoTrans, M-M2, N-N2, R1, Fi.mOne, A3, lda, A2, lda, Fi.one, A4, lda, 0,  method, HPAC_NUM_THREADS);
		  ...
		  // [ H1 H2 ] <- P3^T H Q2^T                                                                                     
		  // [ H3 H4 ]                                               
		  TASK(READ(Fi,Q2), NOWRITE(), READWRITE(A4), papplyP, Fi, FflasRight, FflasTrans, M-M2, 0, N-N2, A4, lda, Q2);
		  //applyP( Fi, FflasRight, FflasTrans, M-M2, 0, N-N2, A4, lda, Q2);
		  WAIT;

		  TASK(READ(Fi,P3), NOWRITE(), READWRITE(A4), papplyP, Fi, FflasLeft, FflasNoTrans, N-N2, 0, M-M2, A4, lda, P3);
		  //applyP( Fi, FflasLeft, FflasNoTrans, N-N2, 0, M-M2, A4, lda, P3);
		  WAIT;
    
      // [ E1 ] <- P3^T E      
      // [ E2 ]
		  TASK(READ(Fi,P3), NOWRITE(), READWRITE(A3), papplyP,  Fi, FflasLeft, FflasNoTrans, R1, 0, M-M2, A3, lda, P3);
		  //applyP(  Fi, FflasLeft, FflasNoTrans, R1, 0, M-M2, A3, lda, P3);
 
      // [ M11 ] <- P2^T M1                                      
      // [ M12 ]
		  TASK(READ(Fi,P2), NOWRITE(), READWRITE(A), papplyP,Fi, FflasLeft, FflasNoTrans, R1, 0, M2-R1, A+R1*lda, lda, P2);
		  //applyP(Fi, FflasLeft, FflasNoTrans, R1, 0, M2-R1, A+R1*lda, lda, P2);

      // [ D1 D2 ] <- D Q2^T 
		  TASK(READ(Fi, Q2), NOWRITE(), READWRITE(A2), papplyP, Fi, FflasRight, FflasTrans, R1, 0, N-N2, A2, lda, Q2);
		  //papplyP( Fi, FflasRight, FflasTrans, R1, 0, N-N2, A2, lda, Q2);

      // [ V1 V2 ] <- V1 Q3^T
		  TASK(READ(Fi, Q3), NOWRITE(), READWRITE(A), papplyP, Fi, FflasRight, FflasTrans, R1, 0, N2-R1, A+R1, lda, Q3);
		  //applyP( Fi, FflasRight, FflasTrans, R1, 0, N2-R1, A+R1, lda, Q3);

      // I <- H1 U2^-1
      // K <- H3 U2^-1                                                      
		  TASK(READ(Fi, R2, F), NOWRITE(), READWRITE(A4), pftrsm, Fi, FflasRight, FflasUpper, FflasNoTrans, Diag, M-M2, R2, Fi.one, F, lda, A4, lda,  method, NUM);
    ...
    // J <- L3^-1 I (in a temp)  
		  TASK(READ(Fi, R2, G), NOWRITE(), READWRITE(temp), pftrsm,  Fi, FflasLeft, FflasLower, FflasNoTrans, OppDiag, R3, R2, Fi.one, G, lda, temp, R2,  method, NUM);
   ...
   // N <- L3^-1 H2                                
		  TASK(READ(Fi, R3, G), NOWRITE(), READWRITE(A4), pftrsm,Fi, FflasLeft, FflasLower, FflasNoTrans, OppDiag, R3, N-N2-R2, Fi.one, G, lda, A4+R2, lda,  method, NUM);
   ...
   // O <- N - J V2
		  TASK(READ(Fi, R2, temp, F), NOWRITE(), READWRITE(A4), pfgemm, Fi, FflasNoTrans, FflasNoTrans, R3, N-N2-R2, R2, Fi.mOne, temp, R2, F+R2, lda, Fi.one, A4+R2, lda,  method, HPAC_NUM_THREADS);
  ...
  // R <- H4 - K V2
		  TASK(READ(Fi, R2, A4, F), NOWRITE(), READWRITE(R), pfgemm, Fi, FflasNoTrans, FflasNoTrans, M-M2-R3, N-N2-R2, R2, Fi.mOne, A4+R3*lda, lda, F+R2, lda, Fi.one, R, lda,  method, HPAC_NUM_THREADS);
		  //fgemm( Fi, FflasNoTrans, FflasNoTrans, M-M2-R3, N-N2-R2, R2, Fi.mOne, A4+R3*lda, lda, F+R2, lda, Fi.one, R, lda);
		  WAIT;
		  
		  delete[] temp;
    // R <- R - M3 O 
		  TASK(READ(Fi, R3, A4, G), NOWRITE(), READWRITE(R), pfgemm, Fi, FflasNoTrans, FflasNoTrans, M-M2-R3, N-N2-R2, R3, Fi.mOne, G+R3*lda, lda, A4+R2, lda, Fi.one, R, lda,  method, HPAC_NUM_THREADS);
    ...
    // [ E22 M32 0 K2 ] 
		  TASK(READ(Fi, P4), NOWRITE(), READWRITE(A3), papplyP, Fi, FflasLeft, FflasNoTrans, N2+R2, 0, M-M2-R3, A3+R3*lda, lda, P4);
    ...
    // [  0   0  ]     [  0 ]                                                  
    // [ O1   O2 ]     [  O ]           
		  TASK(READ(Fi, Q4), NOWRITE(), READWRITE(A2), papplyP, Fi, FflasRight, FflasTrans, M2+R3, 0, N-N2-R2, A2+R2, lda, Q4);
