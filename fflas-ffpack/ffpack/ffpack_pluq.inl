...
#  endif
	...
	PLUQ (const Field& Fi, const FFLAS::FFLAS_DIAG Diag,
	      ...
	      size_t BCThreshold,const ParSeqHelper & PSH)
	{
		Checker_PLUQ<Field> checker (Fi,M,N,A,lda);
		size_t R;
		if (std::is_same<ParSeqHelper,FFLAS::ParSeqHelper::Sequential>::value)
			R = FFPACK::_PLUQ(Fi,Diag,M,N,A,lda,P,Q, BCThreshold);
		else
			R = FFPACK::pPLUQ(Fi,Diag,M,N,A,lda,P,Q, BCThreshold, PSH.numthreads());
