							   ...
							   const FFPACK_LU_TAG LuTag, const ParSeqHelper& PSH)
{
	if (LuTag == FfpackGaussJordan && transform){
		...
		for (size_t i=0; i<M; i++)
			P[i] = i;
		return Protected::GaussJordan(F, M, N, A, lda, 0, 0, N, P, Qt, PSH);
	}
	size_t r;
...
FFPACK::Protected::GaussJordan (const Field& F, const size_t M, const size_t N,
								...
								size_t* P, size_t* Q, const ParSeqHelper& PSH)
{
	...
	size_t r1 = GaussJordan (F, M, N, A, lda, colbeg, rowbeg, recsize, P, Q, PSH);
	...
	applyP (F, FFLAS::FflasLeft, FFLAS::FflasNoTrans, colsize - recsize, rowbeg, rowbeg+r1, A12, lda, P, PSH);
	...
	size_t r2 = GaussJordan (F, M, N, A, lda, colbeg + recsize, rowbeg + r1, colsize - recsize, P, Q, PSH);

	// Apply permutation on A*1
	applyP (F, FFLAS::FflasLeft, FFLAS::FflasNoTrans, r1, rowbeg+r1, rowbeg+r1+r2, A11, lda, P, PSH);
