...
[
        ...
        AC_ARG_ENABLE(avx512, [AC_HELP_STRING([--disable-avx512], [ disable AVX512 instruction set (enabled by default when available)])],[],[])
      ...
      AC_TRY_RUN([
                ...
                AS_IF([ test "$iset" -ge "10" -a "x$enable_avx512" != "xno" ], [
                        AS_ECHO("AVX512 enabled")
                        SIMD_CFLAGS="${SIMD_CFLAGS} -mavx512"
                ],[AS_ECHO("AVX512 disabled")])
        ])
        AC_TRY_RUN([
                        #include "macros/CodeChunk/instrset_detect.cpp"
