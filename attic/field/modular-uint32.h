 ...
 *
 * Written by: extracted/adapted from LinBox by BB <brice.boyer@lip6.fr>
 *
 * ========LICENCE========
...
/*! @file field/modular-uint32.h
 * @ingroup field
 * @brief  representation of <code>Z/mZ</code> over \c uint32_t .
 */

#ifndef __FFLASFFPACK_modular_uint32_H
#define __FFLASFFPACK_modular_uint32_H
...
namespace FFPACK
{
	...
	/** \brief Specialization of Modular to uint32_t element type with efficient dot product.
	 *
	...
	public:
		...
		{
			x = y + z;
			if ((uint32_t) x >= (uint32_t) _modulus) x -= _modulus;
			return x;
		}
		...
		uint32_t &cardinality (uint32_t &c) const
		{
			return c = _modulus;
		}
		...
		uint32_t &characteristic (uint32_t &c) const
		{
			return c = _modulus;
		}
	...
	std::ostream &write (std::ostream &os) const
		{
			return os << "uint32_t mod " << _modulus;
		}
...
#endif //__LINBOX_modular_uint32_H
