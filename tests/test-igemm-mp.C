...
string check_res(size_t m, size_t n, integer* A, size_t lda, const fmpz_mat_t & B){
	...
	for (size_t i=0;i<m;i++)
		for (size_t j=0;j<n;j++){
			...
			if (fmpz_cmp(fmpz_mat_entry(B,i,j),tmp) != 0)
				{
					fmpz_print(fmpz_mat_entry(B,i,j));
					cout<<"=="<<A[i*lda+j]<<endl;
...
int main(int argc, char** argv){
	...
	Integer *A,*B,*C;
	C= FFLAS::fflas_new(Z,m,n);
	A= FFLAS::fflas_new(Z,m,k);
	B= FFLAS::fflas_new(Z,k,n);
	
	for (size_t i=0;i<m;++i)
		for (size_t j=0;j<k;++j)
			integer::random_exact_2exp(A[i*k+j], b);			
	for (size_t i=0;i<k;++i)
		for (size_t j=0;j<n;++j)
			integer::random_exact_2exp(B[i*n+j], b);				
	for (size_t i=0;i<m;++i)
		for (size_t j=0;j<n;++j)
			integer::random_exact_2exp(C[i*n+j], b);			
