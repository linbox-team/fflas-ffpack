...
echo '<report name="report-0" categ="report">' >> $XMLFILE
...
do
	...
	COMPILELOG=$(make $benchmark 2> /dev/null; echo 'Returned state: '$?)
	...
	then
		...
		then
			...
			if [[ $PERFORMANCEFLOPS != "Irrelevant" ]]
				then PERFORMANCEFLOPSRELEVANT='true'
				else PERFORMANCEFLOPSRELEVANT='false'
			fi
			ERRORLOG=''
		fi
	fi
