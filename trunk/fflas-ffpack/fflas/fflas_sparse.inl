...
inline void fspmv(const Field &F, const SM &A,
...
inline void fspmv(const Field &F, const SM &A,
                  ...
                  std::false_type) {
    if (A.delayed) {
        sparse_details::fspmv(F, A, x, y, FieldCategories::UnparametricTag(),
                              std::false_type());
        freduce(F, A.m, y, 1);
    } else {
...
fspmv(const Field &F, const Sparse<Field, SparseMatrix_t::CSR_ZO> &A,
    ...
    if (A.cst == 1) {
        sparse_details_impl::fspmv(
            F, A, x, y, [&F](Element &a, const Element &b) { F.addin(a, b); },
            FieldCategories::GenericTag());
    } else if (A.cst == -1) {
        sparse_details_impl::fspmv(
            F, A, x, y, [&F](Element &a, const Element &b) { F.subin(a, b); },
            FieldCategories::GenericTag());
    } else {
...
fspmv(const Field &F, const Sparse<Field, SparseMatrix_t::CSR_ZO> &A,
    ...
    using Element = typename Field::Element;
    if (std::is_same<SM, Sparse<Field, SparseMatrix_t::ELL_simd>>::value ||
        std::is_same<SM, Sparse<Field, SparseMatrix_t::SELL>>::value) {
#ifdef __FFLASFFPACK_USE_SIMD
        using simd = Simd<Element>;
        if (A.cst == 1) {
            sparse_details_impl::fspmv(F, A, x, y, simd::add,
                                       FieldCategories::UnparametricTag());
        } else if (A.cst == -1) {
            sparse_details_impl::fspmv(F, A, x, y, simd::sub,
                                       FieldCategories::UnparametricTag());
        } else {
            auto x1 = fflas_new(F, A.n, 1, Alignment::CACHE_LINE);
            fscal(F, A.n, A.cst, x, 1, x1, 1);
        ...
        if (A.cst == 1) {
            sparse_details_impl::fspmv(
                F, A, x, y, [](Element &a, const Element &b) { a += b; },
                FieldCategories::UnparametricTag());
        } else if (A.cst == -1) {
            sparse_details_impl::fspmv(
                F, A, x, y, [](Element &a, const Element &b) { a -= b; },
...
fspmv(const Field &F, const Sparse<Field, SparseMatrix_t::CSR_ZO> &A,
    ...
    sparse_details::fspmv<Field, SM>(
        F, A, x, y, FieldCategories::UnparametricTag(), std::true_type());
    freduce(F, A.m, y, 1);
}
...
inline void pfspmv(const Field &F, const SM &A,
                   ...
                   std::false_type) {
    if (A.delayed) {
        sparse_details::pfspmv(F, A, x, y, FieldCategories::UnparametricTag(),
                               std::false_type());
        freduce(F, A.m, y, 1);
    } else {
...
pfspmv(const Field &F, const Sparse<Field, SparseMatrix_t::CSR_ZO> &A,
    ...
    if (A.cst == 1) {
        sparse_details_impl::pfspmv(
            F, A, x, y, [&F](Element &a, const Element &b) { F.addin(a, b); },
            typename FieldCategories::GenericTag());
    } else if (A.cst == -1) {
        sparse_details_impl::pfspmv(
            F, A, x, y, [&F](Element &a, const Element &b) { F.subin(a, b); },
            typename FieldCategories::GenericTag());
...
inline void pfspmv(const Field &F, const SM &A,
                   ...
                   typename Field::Element_ptr y, FieldCategories::ModularTag,
                   std::true_type) {
    sparse_details::pfspmv<Field, SM>(
        F, A, x, y, FieldCategories::UnparametricTag(), std::true_type());
    freduce(F, A.m, y, 1);
}
...
template <class Field, class SM>
inline void fspmm(const Field &F, const SM &A, int blockSize,
    ...
    if (ldx == blockSize && ldy == blockSize) {
        sparse_details_impl::fspmm(F, A, blockSize, x, y,
                                   FieldCategories::GenericTag());
    } else {
        sparse_details_impl::fspmm(F, A, blockSize, x, ldx, y, ldy,
                                   FieldCategories::GenericTag());
    }
}
...
inline void fspmm(const Field &F, const SM &A, int blockSize,
    ...
    if (ldx == blockSize && ldy == blockSize) {
        if (blockSize % simd::vect_size) {
            sparse_details_impl::fspmm(F, A, blockSize, x, y, simd::loadu,
                                       simd::storeu,
                                       FieldCategories::UnparametricTag());
        } else {
            sparse_details_impl::fspmm(F, A, blockSize, x, y, simd::load,
                                       simd::store,
                                       FieldCategories::UnparametricTag());
        }
    } else {
...
inline void fspmm(const Field &F, const SM &A, int blockSize,
    ...
    if (A.delayed) {
        sparse_details::fspmm(F, A, blockSize, x, ldx, y, ldy,
                              FieldCategories::UnparametricTag(),
...
fspmv(const Field &F, const SM &A, typename Field::ConstElement_ptr x,
    ...
    sparse_details::fspmv<Field, SM>(
        F, A, x, y, typename FieldTraits<Field>::category(),
